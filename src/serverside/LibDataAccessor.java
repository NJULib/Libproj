...
public class LibDataAccessor {
	...
	/**
	 * 方法三：取得用户的借阅信息, 包括所有借阅信息 历史借阅信息 当前借阅信息
	 */
	public List[] getBorrowInfo(String readerID) {
		...
		try {
			...
			while (rs1.next()) {
				...
				log("超期天数" + borrowInfo.getOverduedays());
				borrowDataList[0].add(borrowInfo);
			}
			log("读者当前借阅图书：" + borrowDataList[0].size() + "本");
		} catch (SQLException e) {
			log("读取当前借阅图书信息错误：" + e.getMessage());
		} finally {
			try {
				rs1.close();
			} catch (SQLException e) {
				log("关闭结果集rs1错误：" + e.getMessage());
			}
		}
		...
		try {
			...
			log("超期图书：" + borrowDataList[0].size() + "本");
	...
	/**
	 * 方法四：根据管理员的ID和密码查询用户是否存在，存在则返回它的用户名
	 * 
	 * @param libid
	 * @param pass
	 * @return
		...
		try {
			...
			if (rs.next()) {
				...
				libInfo = new LibraianInfo(libid, pass, name, bookp, readerp,
						parameterp);
				log("管理信息:" + libInfo.getBookp());
			} else {
				libInfo = new LibraianInfo();
			}
			log("管理员信息：姓名 "+libInfo.getName());
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return libInfo;
	...
	/**
	 * 方法五：根据读者的的ID和密码查询用户是否存在，存在则返回它的用户名
	 * 
	 * @param readerID
	 * @param pass
	 * @return
	...
	public boolean execBookDataInsert(BookDetails bookDetails) {
		...
		try {
			...
			String sqlBookData = "insert into bookdata(isbn,name,series,authors,publisher,size,pages,price,introduction,picture,clnum) values ('"
					...
					+ "','"
					+ clunm + "')";
			log("执行的sqlBookData为：" + sqlBookData);
	...
	public boolean execBookInfoInsert(BookInLibrary bookInLibrary) {
		...
		String sqlBookInfo = "insert into bookinfo(barcode,isbn,status,location,introducetime) values ('"
			...
			+ ",'"
			+ location+ "','"+introdate+"')";
		log("执行的sqlBookInfo为：" + sqlBookInfo);
	...
	private BookDetails getIsbnBookDetails(String isbn) {
		...
		try {
			...
			log("根据isbn取得图书详细信息出错："+e.getMessage());
		} finally {
			try {
				rs.close();
				stmt.close();
	...
	public boolean execPassModify(String readerid, String pass) {
		...
		try{
			...
			log("修改密码异常"+e.getMessage());
			modify = false;
		}finally{
			try{stmt.close();
			con.close();
			}catch(SQLException e){
				log("修改密码时关闭sql异常："+e.getMessage());
			}
		}
		return modify;
	}
	...
	public boolean execReaderBorrowBook(String readerid, String barCode) {
		boolean mark = false;
		log("接收到读者编号" + readerid + "\t条形码：" + barCode);
	...
	public BorrowInfo returnBook(String barCode) {
		...
		try {
			...
			String upbookinfosql = "update bookinfo set status = "+1+",duedate = null where barcode = '"
				+ barCode + "'";
			log("更新bookinfo"+upbookinfosql);
			int booknum = stmt1.executeUpdate(upbookinfosql);
			log("更新bookinfo"+booknum+"条记录");
			...
			log("更新lendinfo"+uplendinfosql);
			int lendnum = stmt3.executeUpdate(uplendinfosql);
			log("更新lendinfo"+lendnum+"条记录");
...
//			}
			...
			log("借阅信息："+borrowInfo.getReaderId());
		} catch (SQLException e1) {
			log("数据库读异常，" + e1.getMessage());
		}finally{
			try{
				rs.close();
				stmt1.close();
	...
	public Boolean getBarCodeIsBorrowed(String barCode) {
		...
		try {
			...
			mark = false;
			log("检测指定条码的图书是否仍在馆藏中："+e.getMessage());
		}finally{
			try{
				rs.close();
				stmt.close();
	...
	public int getBarcodeIsRenewed(String barCode) {
		...
		try {
			...
			renew = 0;
			log("检测指定条码的图书是否被续借过异常："+e.getMessage());
		}finally{
			try{
				rs.close();
				stmt.close();
